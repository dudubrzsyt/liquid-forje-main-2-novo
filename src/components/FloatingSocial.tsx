...
const items = [
  {
    label: "WhatsApp",
    icon: MessageCircle,
    href: buildWhatsappUrl("Olá Igor! Vim pelo site e quero conversar sobre um projeto."),
  ...
  return (
    <div className="floating-social fixed bottom-3 right-3 z-40 flex flex-col gap-2 sm:bottom-6 sm:right-6 sm:gap-3">
      {items.map(({ label, icon: Icon, href, color }, i) => (
        ...
        >
          <span
            className={`absolute inset-0 rounded-full bg-gradient-to-br ${color} opacity-70 blur-md transition duration-500 group-hover:opacity-100 group-hover:blur-lg`}
          />
          <span
            className={`relative grid h-11 w-11 place-items-center rounded-full bg-gradient-to-br ${color} text-white shadow-xl shadow-black/40 ring-1 ring-white/30 transition-all duration-300 hover:scale-110 hover:-translate-y-1 active:scale-95 sm:h-14 sm:w-14`}
          >
