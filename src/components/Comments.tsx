  ...
  return (
    <section className="mt-14">
      <h2 className="flex items-center gap-2 text-2xl font-black text-white">
        ...
        {rows.length > 0 && (
          <span className="text-base font-medium text-white/50">({rows.length})</span>
        )}
      </h2>
      ...
      {!userId ? (
        <div className="glass-panel mt-5 p-6 text-center">
          <p className="text-white/70 text-sm">
            Entre na sua conta para comentar e participar da comunidade.
          </p>
          <Link
            to="/auth"
            className="mt-4 inline-flex rounded-full bg-gradient-to-r from-primary via-accent to-primary bg-[length:200%_100%] px-5 py-2.5 text-sm font-semibold text-primary-foreground transition hover:bg-[position:100%_0]"
          >
            Entrar
          </Link>
        </div>
      ...
      <div className="mt-6 space-y-3">
        ...
        {!loading && userId && rows.length === 0 && (
          <p className="text-sm text-white/50">Nenhum comentário ainda. Seja o primeiro.</p>
        )}
        {rows.map((c) => {
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  {c.user_id === userId && (
                    <button
                      ...
                      className="ml-auto text-white/40 hover:text-destructive transition"
                    >
