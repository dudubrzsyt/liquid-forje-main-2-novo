...
useEffect(() => {
  ...
  function onPickAvatar(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    if (file.size > 800_000) { toast.error("Imagem muito grande (max 800KB)"); return; }
  ...
  async function save() {
    ...
    try {
      ...
      localStorage.setItem("ie_prefs", JSON.stringify({ name, avatar, newsletter, bio, age, stack, languages }));
  ...
  function togglePhone() {
    setShowPhone((prev) => {
      ...
      localStorage.setItem(
        "ie_contact",
        JSON.stringify({ showPhone: newState, newsletter })
      );
      return newState;
    });
  }
  ...
  if (loading) {
    return <div className="mx-auto max-w-3xl px-4 sm:px-6 py-20 text-white/70 text-center">Carregando...</div>;
  }
