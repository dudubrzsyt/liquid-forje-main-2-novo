  ...
  head: () => ({
    meta: [
      { title: "Comunidade & Marketplace — Diamante.dev" },
      {
        name: "description",
        content:
          "Rede social interna da Diamante.dev: publique projetos, interaja e negocie sites no marketplace.",
      },
      { property: "og:title", content: "Comunidade & Marketplace — Diamante.dev" },
      {
        property: "og:description",
        content: "Poste projetos, curta, comente, siga perfis e anuncie sites à venda.",
      },
...
function ComunidadePage() {
  ...
  const [reactions, setReactions] = useState<
    Record<string, { up: number; down: number; mine: number }>
  >({});
  ...
  useEffect(() => {
    (async () => {
      ...
      setUserId(uid);
      if (uid) {
        await Promise.all([loadFeed(uid), loadMarket()]);
      }
      setLoading(false);
    })();
  }, [loadFeed, loadMarket]);
    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 pb-20">
      ...
      {loading ? (
        <div className="mt-10 flex items-center gap-2 text-white/60">
          <Loader2 className="h-4 w-4 animate-spin" /> Carregando comunidade...
        </div>
      ) : tab === "feed" ? (
        <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
          <div>
            ...
            <div className="mt-6 space-y-5">
              {posts.length === 0 && (
                <p className="glass-panel p-8 text-center text-white/65">
          ...
          <aside className="space-y-5 lg:sticky lg:top-24 lg:self-start">
            ...
            <div className="glass-panel p-6">
              ...
              <Link
                to="/perfil"
                className="mt-5 inline-flex rounded-full glass px-4 py-2 text-xs font-semibold text-white"
              >
                Editar meu perfil
              </Link>
            </div>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            {listings.length === 0 && (
              <p className="glass-panel p-8 text-center text-white/65 sm:col-span-2">
                Nenhum site à venda ainda.
              </p>
            )}
            {listings.map((l) => (
...
function Avatar({ url, size = 10 }: { url?: string | null; size?: number }) {
  return (
    <span className="relative grid place-items-center shrink-0">
      ...
      <span
        className={`relative m-[2px] grid place-items-center overflow-hidden rounded-full bg-[oklch(0.14_0.07_260)] ring-2 ring-white/10`}
  ...
  async function publish() {
    ...
    if (file) {
      if (file.size > 25 * 1024 * 1024) {
        toast.error("Arquivo acima de 25MB");
        setBusy(false);
        return;
      }
    ...
    const { error } = await supabase
      .from("community_posts")
      .insert({ user_id: userId, content: content.slice(0, 2000), media_url, media_type });
    setBusy(false);
    if (error) {
      toast.error(error.message);
      return;
...
function PostCard({
  ...
  mediaUrl?: string;
  mine: boolean;
  stats: { up: number; down: number; mine: number };
  following: boolean;
  onReact: (id: string, v: number) => void;
    ...
    <article className="glass-panel overflow-hidden">
      <header className="flex items-center gap-3 p-5">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="truncate text-xs text-white/50">
            @{author?.username ?? "diamante"} · {timeAgo(post.created_at)}
          </div>
        </div>
        {!mine && (
          <button
...
function ListingForm({ userId, onDone }: { userId: string; onDone: () => void }) {
  ...
  const input =
    "w-full rounded-xl border border-white/15 bg-white/5 px-4 py-3 text-white placeholder:text-white/40 outline-none transition focus:border-primary focus:ring-2 focus:ring-primary/30";

  return (
    <form
      ref={formRef}
      onSubmit={submit}
      className="glass-panel h-fit space-y-3 p-6 lg:sticky lg:top-24"
    >
      ...
      <textarea
        name="description"
        rows={3}
        placeholder="Descrição, stack, o que está incluso..."
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <select
          ...
          className={`${input} [&>option]:bg-[oklch(0.16_0.08_260)]`}
        >
