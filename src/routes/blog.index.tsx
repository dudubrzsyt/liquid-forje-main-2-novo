...
export const Route = createFileRoute("/blog/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Artigos sobre desenvolvimento, performance, design e conversão para quem quer um site que gera resultado." },
      { property: "og:title", content: "Blog — Diamante.dev" },
      { property: "og:description", content: "Conteúdo técnico e prático sobre criação de sites, performance e vendas online." },
...
function BlogIndex() {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 sm:px-6 pb-20">
      <section className="pt-8 pb-10 md:pt-14">
        <Reveal className="max-w-3xl">
          ...
          <p className="mt-4 text-lg text-white/70">
            Bastidores técnicos, decisões de arquitetura e o que realmente move o ponteiro de faturamento em um site.
          </p>
        </Reveal>
            ...
            {BLOG_CATEGORIES.map((c) => (
              <button
                ...
                className={`rounded-full px-4 py-2 text-sm font-medium transition-all duration-300 ${
                  cat === c ? "bg-gradient-to-r from-primary to-accent text-primary-foreground shadow-lg shadow-primary/25" : "glass text-white/80 hover:-translate-y-0.5"
                }`}
              >
                {c}
              </button>
      ...
      {posts.length === 0 && (
        <p className="glass-panel p-10 text-center text-white/70">Nenhum artigo encontrado para essa busca.</p>
      )}

      {featured && (
        <Reveal>
          <Link to="/blog/$slug" params={{ slug: featured.slug }} className="group block glass-panel overflow-hidden hover-lift">
            <div className="grid md:grid-cols-2">
              <div className="relative aspect-[16/10] overflow-hidden">
                <img src={featured.cover} alt={featured.title} width={1280} height={720} loading="lazy"
              ...
              <div className="p-7 md:p-10 flex flex-col justify-center">
                ...
                <h2 className="mt-3 text-2xl md:text-3xl font-black text-white leading-tight">{featured.title}</h2>
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {rest.map((p, i) => (
          <Reveal key={p.slug} delay={i * 60}>
            <Link to="/blog/$slug" params={{ slug: p.slug }} className="group flex h-full flex-col glass-panel overflow-hidden hover-lift">
              <div className="aspect-[16/9] overflow-hidden">
                <img src={p.cover} alt={p.title} width={1280} height={720} loading="lazy"
      ...
      <section className="mt-24">
        <Reveal className="text-center max-w-2xl mx-auto mb-8">
          <h2 className="text-3xl md:text-4xl font-black text-white">Gostou do conteúdo? <span className="text-gradient">Vamos aplicar</span> no seu site.</h2>
