  ...
  head: () => ({
    meta: [
      { title: "Pacotes e Preços — Diamante.dev" },
      {
        name: "description",
        content:
          "Landing Page, Institucional, E-commerce, Premium e Corporativo. Tabela clara de preços e o que inclui.",
      },
      { property: "og:title", content: "Pacotes e Preços — Diamante.dev" },
      {
        property: "og:description",
        content:
          "Escolha o site ideal para o seu negócio: da landing rápida ao e-commerce completo.",
      },
    ],
  }),
  component: Vendas,
      ...
      <section className="pt-8 pb-16 text-center">
        <Reveal>
          ...
          <p className="mt-4 max-w-2xl mx-auto text-white/70">
            Do landing rápido ao corporativo. Todos os pacotes incluem design sob medida, código
            próprio e responsividade total.
          </p>
        </Reveal>
      </section>
      ...
      <section className="pb-16 grid gap-5 md:grid-cols-2 lg:grid-cols-3">
        {pacotes.map((p, i) => (
          <Reveal key={p.name} delay={i * 70} variant="zoom" className="h-full">
            <div
              className={`glass-panel relative p-7 flex flex-col h-full hover-lift ${p.tag === "Popular" ? "ring-2 ring-primary/60 animate-pulse-glow" : ""}`}
            >
              {p.tag && (
              ...
              {[
                ...
                >
                  {row.map((c, i) => (
                    <td
                      key={i}
                      className={`p-4 ${i === 3 ? "text-right font-semibold text-gradient" : ""}`}
                    >
                      {c}
                    </td>
                  ))}
        ...
        <div className="grid gap-10 lg:grid-cols-2 items-center">
          <Reveal variant="left">
            ...
            <p className="mt-3 text-white/70 max-w-lg">
              Me conta sua ideia e escolha o canal preferido. Envio direto no meu WhatsApp e
              respondo em horas.
            </p>
          </Reveal>
