  ...
  head: () => ({
    meta: [
      { title: "Cadastre-se como Dev — Diamante.dev" },
      {
        name: "description",
        content:
          "Envie seu perfil, tenha o GitHub analisado por IA e receba seu score em diamantes para aparecer no ranking de devs da Diamante.dev.",
      },
      { property: "og:title", content: "Cadastre-se como Dev — Diamante.dev" },
      {
        property: "og:description",
        content: "Análise de GitHub com IA, score em diamantes e ranking de desenvolvedores.",
      },
...
function CadastroDevPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setResult(res);
        toast.success(
          "Análise concluída: seu perfil está ativo e validado pela inteligência da plataforma. ",
        );
      }
    } catch {
      toast.error(
        "Ops! Confira os campos: a biografia deve ter entre 30 e 40 caracteres e os links precisam estar corretos.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const field =
    "w-full rounded-xl border border-white/15 bg-white/60 px-4 py-3 text-sm text-slate-900 outline-none transition-all duration-300 placeholder:text-slate-400 focus:border-sky-400/60 focus:bg-white focus:ring-4 focus:ring-sky-400/15 dark:bg-white/10 dark:text-white";
    ...
    <div className="mx-auto max-w-5xl px-4 pb-24 pt-28 sm:px-6 sm:pt-32">
      <Reveal>
        ...
        <p className="mt-3 max-w-2xl text-base opacity-70">
          Seu GitHub é verificado e analisado por IA para gerar um score de confiabilidade em
          diamantes. Quanto melhor o histórico público e o plano de assinatura, mais alto você
          aparece no ranking.
        </p>
      </Reveal>
      ...
      <Reveal delay={80}>
        <div className="mt-6 flex flex-wrap items-center gap-3 rounded-2xl glass-panel p-4 text-sm">
          ...
          <span className="font-semibold">
            Seu perfil será analisado pela IA em até 30–60 minutos.
          </span>
      ...
      {!userId ? (
        <Reveal delay={160}>
          <div className="mt-10 rounded-3xl glass-panel p-8 text-center">
            ...
            <p className="mt-2 text-sm opacity-70">
              Precisamos de uma conta para vincular seu score, assinatura e histórico.
            </p>
            <Link
              to="/auth"
        ...
        <Reveal delay={160}>
          <form
            onSubmit={onSubmit}
            className="mt-10 grid gap-4 rounded-3xl glass-panel p-6 sm:p-8 md:grid-cols-2"
          >
            ...
            <label className="grid gap-1.5 text-sm font-semibold md:col-span-2">
              ...
              <textarea
                ...
                value={form.bio}
                onChange={set("bio")}
                placeholder="Conte sua experiência, projetos e o que você entrega melhor."
              />
            </label>
            <label className="grid gap-1.5 text-sm font-semibold md:col-span-2">
              ...
              <input
                ...
                value={form.stack}
                onChange={set("stack")}
                placeholder="React, TypeScript, Node, PostgreSQL"
              />
            </label>
            <label className="grid gap-1.5 text-sm font-semibold md:col-span-2">
              ...
              <textarea
                ...
                placeholder="Ciência da Computação — USP; AWS Solutions Architect"
              />
            </label>
            <label className="grid gap-1.5 text-sm font-semibold">
              <span className="inline-flex items-center gap-1.5">
                <Github size={14} /> Link do GitHub
              </span>
              <input
                required
                className={field}
            ...
            <label className="grid gap-1.5 text-sm font-semibold">
              <span className="inline-flex items-center gap-1.5">
                <Linkedin size={14} /> Link do LinkedIn
              </span>
              <input
                required
                type="url"
            ...
            >
              {loading ? (
                <>
                  <Loader2 className="animate-spin" size={16} /> Analisando seu GitHub com IA…
                </>
              ) : (
                <>Enviar para análise</>
              )}
      ...
      {result && (
        <Reveal>
          <div className="mt-8 rounded-3xl glass-panel p-6 sm:p-8">
                ...
                <div className="text-sm opacity-70">
                  {result.approved ? (
                    <span className="inline-flex items-center gap-1.5 text-emerald-600">
                      <CheckCircle2 size={14} /> Perfil aprovado
                    </span>
                  ) : (
            ...
            <Link
              to="/assinaturas"
              className="mt-5 inline-flex rounded-xl bg-green-500 px-5 py-3 text-sm font-semibold text-white transition-transform hover:scale-105"
            >
              <svg
